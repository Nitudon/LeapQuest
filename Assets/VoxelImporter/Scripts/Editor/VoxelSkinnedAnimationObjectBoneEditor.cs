	...
	public class VoxelSkinnedAnimationObjectBoneEditor : Editor
	{
        ...
        void OnDisable()
        {
            ...
            if(!AnimationMode.InAnimationMode())
            {
                ...
                {
                    objectCore.ReCreate();
                    objectTarget.edit_afterRefresh = false;
                }
                else
                {
                ...
                else if (objectTarget.editMode == VoxelSkinnedAnimationObject.Edit_Mode.BoneWeight)
                {
                    if (boneTarget == rootTarget)
                    {
                        #region Tool
                        {
                            ...
                            #region Global
                            {
                                var editorBoxRect = new Rect(2, 2, 204, 60);
                            ...
                            #region WeightPreviewMode
                            {
                                ...
                                {
                                    EditorGUI.BeginChangeCheck();
                                    var edit_WeightPreviewMode = (VoxelSkinnedAnimationObject.Edit_WeightPreviewMode)EditorGUI.EnumPopup(new Rect(x + 100, y, 200 - 100, 16), objectTarget.edit_WeightPreviewMode);
                                    if (EditorGUI.EndChangeCheck())
                                    {
                            ...
                            #region WeightClear
                            {
                                if (GUI.Button(new Rect(x, y, 200, 16), "Clear All Bones Weight"))
                                {
                                    ...
                                    Undo.RecordObjects(objectTarget.bones, "Clear");
                    ...
                    else
                    {
                        ...
                        #region Tool
                        {
                            ...
                            float y = 20;
                            {
                                var editorBoxRect = new Rect(2, 2, 204, 240);
                                GUI.Box(editorBoxRect, "Bone Weight Editor", editorCommon.guiStyleAlphaBox);
                                editorCommon.editorRectList.Add(editorBoxRect);
                            }
                            #region VoxelMode
