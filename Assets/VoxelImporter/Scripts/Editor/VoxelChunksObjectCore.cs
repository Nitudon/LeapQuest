...
namespace VoxelImporter
{
    public class VoxelChunksObjectCore : VoxelBaseCore
    {
        ...
        {
            voxelBase.voxelData.chunkTable = new DataTable3<IntVector3>(voxelObject.voxelData.voxelSize.x, voxelObject.voxelData.voxelSize.y, voxelObject.voxelData.voxelSize.z);
            ...
            for (int i = 0; i < voxelObject.voxelData.voxels.Length; i++)
            {
                ...
                //
                voxelBase.voxelData.chunkTable.Set(voxelObject.voxelData.voxels[i].position, chunkPosition);
            }
            {
            ...
            if (voxelObject.splitMode == VoxelChunksObject.SplitMode.QubicleMatrix)
            {
                ...
                #region CreateChunkData
                {
                    ...
                    for (int i = 0; i < voxelObject.voxelData.voxels.Length; i++)
                    {
                        ...
                        //
                        voxelBase.voxelData.chunkTable.Set(voxelObject.voxelData.voxels[i].position, new IntVector3(chunkIndex, int.MinValue, int.MinValue));
                    }
                    {
                        chunkDataList = new List<ChunkData>(chunkVoxels.Count);
                        var enu = chunkVoxels.GetEnumerator();
                        ...
                        chunkDataList.Sort((a, b) => string.Compare(chunkNameList[a.position.x], chunkNameList[b.position.x]));
                    }
                }
                #endregion
        ...
        {
            return voxelBase.voxelData.chunkTable.Get(basePos) == voxelBase.voxelData.chunkTable.Get(combinePos);
        }
